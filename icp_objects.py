...
def transform_pcs(input_dataset_folder, source_idx, target_idx):
  ...
  source_masks = joblib.load(f"{input_dataset_folder}/{source_idx:06d}_classes.pkl")
  
  target_np = joblib.load(f"{input_dataset_folder}/{target_idx:06d}_pc.pkl")
  target_pc = np_to_pc(target_np)
  target_masks = joblib.load(f"{input_dataset_folder}/{target_idx:06d}_classes.pkl")

  object_ids = np.intersect1d(np.unique(source_masks), np.unique(target_masks))
  ...
  source_pcs = [extract_object_by_id(source_pc, source_masks, oid) for oid in object_ids]
  target_pcs = [extract_object_by_id(target_pc, target_masks, oid) for oid in object_ids]
  transformed_target_pcs = [get_transformed_target(spc, tpc) for spc, tpc in zip(source_pcs, target_pcs)]
  source_bg_pc_pts = extract_background_np(source_pc, source_masks)
  transformed_target_pc_with_source_bg = np_to_pc(np.concatenate([source_bg_pc_pts] + [pc_to_np(pc) for pc in transformed_target_pcs]))
...
def process_idx(input_dataset_folder, output_dataset_folder, idx):
  ...
  source_pc, transformed_target_pc = transform_pcs(input_dataset_folder, idx, idx + 1)
  joblib.dump(pc_to_np(source_pc), f"{output_dataset_folder}/input/{idx:06}.pkl")
  joblib.dump(pc_to_np(transformed_target_pc), f"{output_dataset_folder}/pair/{idx:06}.pkl")


parser = argparse.ArgumentParser()
parser.add_argument('input_dataset_folder', help='Folder with the created dataset')
parser.add_argument('output_dataset_folder', help='Folder with the created dataset')
...
with multiprocessing.Pool(args.num_cpus) as p:
  p.map(functools.partial(process_idx, input_dataset_folder, output_dataset_folder), 
        range(len(source_files) - 1))
